...
class BLIMPConfig:
    """
    Configuration for BLIMP.

    Defines device-specific ``EXPERIMENT_DIR`` and ``BASE_DATA_DIR`` paths and
    mapping of data configuration names to configuration files.

    The configuration is attempted to be read from a ``blimp.ini`` configuration file.
    ...
    # TODO: implement
    A default configuration file can be created using ``blimp setup``.
    ...
    def _discover_config_file(self) -> Optional[Union[Path, str]]:
        ...
        config_fnames = [
            ...
            Path.home() / ".config" / "blimp" / "blimp.ini",
        ]
    ...
    def __str__(self):
        s = f"BLIMPConfig (fname: {self.config_fname})"
    ...
    def EXPERIMENT_DIR(self):
        ...
        if self._EXPERIMENT_DIR is None:
            ...
            s += ' Please create a config with "blimp setup" or set blimp_config.EXPERIMENT_DIR manually.'
            print(s)
    ...
    def BASE_DATA_DIR(self):
        ...
        if self._BASE_DATA_DIR is None:
            ...
            s += ' Please create a config with "blimp setup" or set blimp_config.BASE_DATA_DIR manually.'
            print(s)
    ...
    def add_data_config(self, data_config_name: str, data_config_file: str) -> None:
        """
        Add a (name, file) mapping to :attr:`BLIMPConfig.data_configs`.
    ...
    def write(self, config_fname=None):
        ...
        ----------
        config_fname
            Name of the configuration file. Default is :attr:`BLIMPConfig.config_fname`.
        """
...
# blimp config instance (to be used by all scripts)

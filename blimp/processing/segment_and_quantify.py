...
def segment_nuclei_cellpose(
    ...
    if timepoint is None:
        nuclei_images = [intensity_image.get_image_data("ZYX", C=nuclei_channel, T=t) for t in range(intensity_image.dims[['T']][0])]
    else:
...
def segment_and_quantify(
    ...
    timepoint : Union[int,None] = None) -> None:
    """
    ...
    if metadata_file is None:
        ...
        logging.warning("Metadata file not provided, using default location: {}".format(str(metadata_file)))
    else:
        metadata_file = Path(metadata_file)
    
    if not metadata_file.exists():
        logging.error("No metadata file provided: {} does not exist".format(str(metadata_file)))

...
def test_calculate_shifts_output_types(_ensure_test_data):
    ...
    result_parameters = blimp.preprocessing.registration.calculate_shifts(test_images, 0, 0)
...
def test_apply_shifts(_ensure_test_data):
    test_images = _load_test_data("operetta_cls_multiplex")
    ...
    with pytest.raises(ValueError):
        blimp.preprocessing.registration.apply_shifts(
            test_images, transformation_parameters=[(0, 0), (2, 2)], crop=True
        )
